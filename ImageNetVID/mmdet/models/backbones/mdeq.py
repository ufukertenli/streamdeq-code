...
sys.path.append("lib/")
from .deq_utils.utils import get_world_size, get_rank

from .deq_config import deq_config
from .deq_config import update_config
...
from .lib.layer_utils import list2vec, vec2list, conv3x3, conv5x5
...
class MDEQNet(nn.Module):
    ...
    def __init__(self, deq_cfg, **kwargs):
        """
        ...
        print(f'MDEQ init from config {deq_cfg}')
        update_config(deq_config, deq_cfg)
        self.parse_cfg(deq_config)
        ...
        if self.downsample_times > 2:
            for i in range(3, self.downsample_times + 1):
                ...
                self.downsample.add_module(f"DS{i}-BN",
                                           nn.BatchNorm2d(init_chansize, momentum=BN_MOMENTUM, affine=True))
    ...
    def parse_cfg(self, cfg):
        ...
        self.pretrain_jac_loss_weight = cfg['LOSS']['PRETRAIN_JAC_LOSS_WEIGHT']
        self.jac_stop_iter = cfg['LOSS']['JAC_STOP_ITER']
        self.jac_loss_weight = cfg['LOSS']['JAC_LOSS_WEIGHT']
        self.update_freq = cfg['LOSS']['JAC_INCREMENTAL']
        self.jac_loss_freq = cfg['LOSS']['JAC_LOSS_FREQ']
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            print(f'Loading backbone from {pretrained}')
    ...
    def forward(self, x_, train_step=-1, compute_jac_loss=False, spectral_radius_mode=False, **kwargs):
        """
        ...
        deq_mode = ((train_step < 0) or (train_step >= self.pretrain_steps)) and deq_m
